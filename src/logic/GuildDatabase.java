...
public class GuildDatabase {
	...
	public boolean createDepartment(String name) {
		...
		if (this.isExists(name)) {
			...
			Department ndep = new Department(name);
			myDepartments.add(ndep);
			return true;
		}
	...
	public boolean isExists(String name) {
		...
		for (int i = 0; i < myDepartments.size(); i++) {
			if (myDepartments.get(i).getName().equals(name)) {
				return true;
			}
		}
		return false;
	...
	public ArrayList<GuildMember> removeDepartment(int index) {
		...
		ArrayList<GuildMember> memlist = myDepartments.get(index).getAllMembers();
